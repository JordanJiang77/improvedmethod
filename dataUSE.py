...
dataDirect0 = pd.DataFrame()
for index, row in dataChoose.iterrows():
    for i in range(0, 20, 5):
        time = row[3]
        if time == i:
            oneData = {"time": row[3], "roadsection": row[2], "numDirect0": row[0]}
            dataDirect0 = dataDirect0.append(oneData, ignore_index=True)
pd.set_option('display.max_rows', None)
print(dataDirect0)

# 使用data的list形式
Datatrain0 = pd.DataFrame()
for index, row in dataChoose.iterrows():
    for i in range(0, 20, 5):
        time = row[3]
        if time == i:
            oneData = {"numDirect0": row[0]}
            Datatrain0 = Datatrain0.append(oneData, ignore_index=True)
pd.set_option('display.max_rows', None)
# print(Datatrain0)


Datatest0 = pd.DataFrame()
for index, row in dataChoose.iterrows():
    for i in range(15, 20, 5):
        time = row[3]
        if time == i:
            oneData = {"numDirect0": row[0]}
            Datatest0 = Datatest0.append(oneData, ignore_index=True)
pd.set_option('display.max_rows', None)
# print(Datatest0)
